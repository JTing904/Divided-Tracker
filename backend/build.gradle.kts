...
dependencies {
    ...
    // Google's own ID-token verifier. Checking an RS256 signature by hand means fetching and
    // caching JWKS, picking the key by kid and refusing an attacker-chosen algorithm; this is
    // the wrong place to be inventive, so the verification Google publishes is the one used.
