    ...
    Column(
        ...
        DsCard(modifier = Modifier.fillMaxWidth(), contentPadding = PaddingValues(20.dp)) {
            DsTextField(
                ...
                isError = state.error?.fieldErrors?.containsKey("name") == true,
                supportingText = state.error?.fieldErrors?.get("name"),
            )
            ...
            DsTextField(
                ...
                isPassword = true,
                isError = state.passwordTooShort || state.error?.fieldErrors?.containsKey("password") == true,
                supportingText = when {
                    state.passwordTooShort -> "Use at least 8 characters"
                    else -> state.error?.fieldErrors?.get("password")
                },
            )
